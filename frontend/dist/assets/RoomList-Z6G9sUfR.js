  ...
  `,document.body.appendChild(e);let t=e.querySelector(`.lb-img`),n=()=>{e.classList.contains(`active`)&&(e.classList.remove(`active`),t.src=``,history.state?.__lb&&history.back())};e.querySelector(`.lb-backdrop`).addEventListener(`click`,n),e.querySelector(`.lb-close`).addEventListener(`click`,n),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&n()}),window.addEventListener(`popstate`,n=>{e.classList.contains(`active`)&&(e.classList.remove(`active`),t.src=``)});let r=0;e.addEventListener(`touchstart`,e=>{r=e.touches[0].clientY},{passive:!0}),e.addEventListener(`touchend`,e=>{e.changedTouches[0].clientY-r>80&&n()},{passive:!0}),window.__openLightbox=(n,r=``)=>{t.src=n,t.alt=r,e.classList.add(`active`),history.pushState({__lb:!0},``)}}var h=class{constructor(e,t){this.container=e,this.roomId=t,this._render()}_render(){this.container.innerHTML=`
      <div class="message-input-area">
        ...
        <div class="message-input-row">
          <button class="attach-btn" id="photo-btn-${this.roomId}" title="사진 첨부" style="color: var(--color-text);">
          ...
          <input type="file" id="image-input-${this.roomId}" accept="image/*,video/*" style="display:none" />
          <input type="file" id="file-input-${this.roomId}" accept="*/*" style="display:none" />
          <textarea
    ...
    `;let e=document.getElementById(`msg-input-${this.roomId}`),t=document.getElementById(`msg-send-${this.roomId}`),n=document.getElementById(`photo-btn-${this.roomId}`),r=document.getElementById(`attach-btn-${this.roomId}`),a=document.getElementById(`image-input-${this.roomId}`),s=document.getElementById(`file-input-${this.roomId}`),c=document.getElementById(`attach-preview-${this.roomId}`),l=null,u=!1,d=()=>{if(u)return;u=!0;let t=e.value.trim();if(!t){u=!1;return}e.value=``,e.style.height=`auto`;try{i(this.roomId,t)}finally{setTimeout(()=>{u=!1},200)}},f=e=>{let t=(e.size/1024).toFixed(1)+` KB`,n=e.type.startsWith(`image/`)?`<img src="${URL.createObjectURL(e)}" style="max-height:80px;border-radius:6px;" />`:`<span style="font-size:24px;color:var(--color-text-muted);"><i class="ph ph-paperclip"></i></span>`;c.innerHTML=`
        ${n}
        <span class="file-info">${g(e.name)} (${t})</span>
        <button class="attach-cancel" id="cancel-attach-${this.roomId}">✕</button>
      `,c.style.display=`flex`,document.getElementById(`cancel-attach-${this.roomId}`)?.addEventListener(`click`,()=>{l=null,s&&(s.value=``),a&&(a.value=``),c.style.display=`none`,c.innerHTML=``})},p=async()=>{if(!l||u)return;u=!0;let e=l;l=null,t.disabled=!0,r&&(r.disabled=!0),n&&(n.disabled=!0),c.innerHTML=`
        <span class="upload-spinner"></span>
        <span class="file-info">${g(e.name)} 보내는 중…</span>
      `,c.style.display=`flex`;let i=new FormData;i.append(`file`,e);try{await o.uploadFile(this.roomId,i),c.style.display=`none`,c.innerHTML=``,s&&(s.value=``),a&&(a.value=``)}catch(t){alert(`업로드 실패: ${t.message}`),l=e,f(e)}finally{u=!1,t.disabled=!1,r&&(r.disabled=!1),n&&(n.disabled=!1)}};e.addEventListener(`keydown`,e=>{if(e.key===`Enter`&&!e.shiftKey){if(e.preventDefault(),e.isComposing||e.keyCode===229||u)return;l?p():d()}}),e.addEventListener(`input`,()=>{e.style.height=`auto`,e.style.height=Math.min(e.scrollHeight,140)+`px`}),t.addEventListener(`click`,e=>{e.preventDefault(),l?p():d()}),document.getElementById(`voice-btn-${this.roomId}`)?.addEventListener(`click`,()=>{e.value=`안녕하세요, 음성으로 말하고 있습니다. (인식 완료)`,e.style.height=`auto`,e.style.height=e.scrollHeight+`px`,e.focus()}),n?.addEventListener(`click`,()=>a.click()),r?.addEventListener(`click`,()=>s.click());let m=e=>{let t=e.files[0];if(!t)return;let n=t.type.startsWith(`image/`)?10*1024*1024:50*1024*1024;if(t.size>n){let n=t.type.startsWith(`image/`)?`10MB`:`50MB`;alert(`파일 크기가 ${n}를 초과합니다.`),e.value=``;return}l=t,f(t)};a?.addEventListener(`change`,()=>m(a)),s?.addEventListener(`change`,()=>m(s))}};function g(e){return String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}var _=class{constructor(e,t,n){this.container=e,this.roomId=t,this.user=n,this.room=null,this.msgList=null,this.threadPanel=null,this._onOpenThread=e=>{e.detail.roomId===this.roomId&&this._openThread(e.detail.message)},document.addEventListener(`open_thread`,this._onOpenThread),this._unsubRoomUpdated=r(`room_updated`,({roomId:e,name:t})=>{if(e===this.roomId&&this.room){this.room.name=t;let n=document.getElementById(`chat-room-name-${this.roomId}`);n&&(n.textContent=t);let r=document.querySelector(`.room-item[data-room-id="${e}"] .room-name`);r&&(r.textContent=t)}}),this._load()}async _load(){try{this.room=await o.getRoom(this.roomId),this._render(this.room),t(this.roomId)}catch(e){this.container.innerHTML=`<p class="error">채팅방을 열 수 없습니다: ${e.message}</p>`}}_render(e){let t=this.user.isOwner;this.container.innerHTML=`
